...
export const Connect = () => {
  ...
  const walletConnect = async() =>{
    ...
    const defaultAccount = await web3Enable('polkadot-extension-example')
    .then(async () => {
      return await web3Accounts();
    })
    if(defaultAccount.length > 0){
        setState({...state, data: { accounts: defaultAccount, defaultAccount: defaultAccount[0]}}) 
    }else {
        setState({...initialExtensionState, loading: false, error: new Error('PolkaConnect not found') });
    }
}
  ...
  return state.data
    ...
    : <button className="text-white bg-purple-600 hover:bg-purple-700 px-4 py-2 rounded-lg" disabled={state.loading} onClick={walletConnect}>
